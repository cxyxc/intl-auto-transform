export const submit = () => (dispatch, getState) => {
    const assignment = getState()
        .getIn(['page', 'appState', 'assignment'])
        .toJS();
    const emptyErrors = [];

    const string = `${A}客户${B}`;

    if(!assignment || !assignment.customerId) emptyErrors.push('客户');
    if(!assignment || !assignment.type) emptyErrors.push('任务类型');
    if(!assignment || !assignment.expectedTime) emptyErrors.push('预计回访时间');
    if(!assignment || !assignment.expiredTime) emptyErrors.push('关闭时间');
    if(!assignment || !assignment.contactMethod) emptyErrors.push('回访方式');
