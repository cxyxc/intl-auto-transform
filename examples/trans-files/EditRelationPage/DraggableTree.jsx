...
class CustomerTreeNode extends PureComponent {
    ...
    onMouseEnter = () => {
        if(!this.props.isOver) this.setState({hover: true});
    };
    onMouseLeave = () => {
        if(!this.props.isOver) this.setState({hover: false});
    };
    render() {
        ...
        return connectDropTarget(
            <div
                ...
                {data.username}
                {this.state.hover && (
                    <Icon type="delete" className={styles.delete_icon} onClick={this.onClickDeleteIcon} />
                )}
            </div>
        );
    }
...
const DropTargetNode = DropTarget('employee', treeNodeTarget, collect)(
    CustomerTreeNode
);
class DraggableTree extends PureComponent {
    ...
    renderTreeNodes = treeData =>
        treeData.map(item => {
            ...
            const title = (
                <DropTargetNode data={data}
                    onDelete={this.props.deleteRelation}
                    onAdd={this.props.addRelation} />
            );
    ...
    onSelect = selectedKeys => {
        this.props.onModifyState({selectedKeys});
    }
    render() {
...
export default connect(mapStateToProps, mapDispatchToProps)(localize(DraggableTree));
