...
export const getCurrentUserPagePermissions = pageCode => dispatch => {
    ...
    return getCurrentUserPagePermissionsApi(pageCode).then(data => {
        if(data.isOk)
            dispatch(getCurrentUserPagePermissionsSuccess(data.data, pageCode));
        else
            dispatch(getCurrentUserPagePermissionsFail(data.statusCode, data.message, pageCode));
    });
};
...
const getEmployees = condition => dispatch => {
    ...
    return fetch(`${baseUrl}/employees?${queryCondition}`).then(res => {
        if(res.ok)
...
export const onClickSearchEmployeeBtn = () => (dispatch, getState) => {
    const condition = getState().getIn(['page', 'uiState', 'queryPanel']).toJS();
...
const updateEmployeeSuccess = () => ({
    ...
    message: getString('DEALER_INFORMATION_EDIT_SUCCESS'),
...
export const updateEmployee = id => (dispatch, getState) => {
    ...
    const requiredField = [];
    if(isNullOrUndefined(tmpEmployeeInfo.sex))
        requiredField.push(getString('SEX'));
    if(isNullOrUndefined(tmpEmployeeInfo.job))
        requiredField.push(getString('BUSINESS_POST'));
    if(isNullOrUndefined(tmpEmployeeInfo.dealerId))
        requiredField.push(getString('BRANCH'));
    if(requiredField.length > 0) {
        dispatch(updateEmployeeFail(null, `${requiredField.join('，')}${getString('REQUIRED')}`));
...
export const getEmployeeDetail = id => dispatch => makeApiCall({
    url: `${baseUrl}/employees/${id}`,
...
const clearUpRelations = (relations, employees) => {
    const result = relations.reduce((result, item) => {
        if(employees.findIndex(employee => employee.userId === item.userId) >= 0 &&
...
export const getAllEmployees = () => dispatch => makeApiCall({
...
const submitRelationInfoSuccess = () => ({
    ...
    message: getString('DEALER_RELATION_EDIT_SUCCESS'),
    notificationType: NOTIFICATION_TYPE.success,
    timeStamp: Date.now().toString()
});
