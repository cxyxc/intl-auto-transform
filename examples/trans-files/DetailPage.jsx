        ...
        return (
            <div>
                <Card loading={isFetching}>
                    <DescriptionList size="large" title={getString('BASIC_INFORMATION')} style={{marginBottom: 32}}>
                        <Description term={getString('USERNAME')}>{data.username}</Description>
                        <Description term={getString('NAME')}>{data.name}</Description>
                        <Description term={getString('SEX')}>
                            {conventEnumValueToString(sex, data.sex)}
                        </Description>
                        <Description term={getString('JOB')}>
                            {conventEnumValueToString(jobType, data.job)}
                        </Description>
                    ...
                    <Divider />
                    <DescriptionList size="large" title={getString('PERSONNEL_INFORMATION')} style={{marginBottom: 32}}>
                        ...
                        <Description term={getString('BIRTHDAY')}>
                            {formatDateTimeStr(data.birthday, DATA_FORMAT)}
                        </Description>
                        <Description term={getString('EDUCATION')}>
                            {conventEnumValueToString(educationType, data.education)}
                        </Description>
                        <Description term={getString('ETHNIC')}>{data.ethnic}</Description>
                        <Description term={getString('POLITICAL')}>
                            {conventEnumValueToString(politicalType, data.political)}
                        </Description>
                ...
                <div className="page-toolbar">
                    <Button type="primary" onClick={this.onReturn}>{getString('RETURN')}</Button>
                </div>
            </div>
        );
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(localize(DetailPage));
